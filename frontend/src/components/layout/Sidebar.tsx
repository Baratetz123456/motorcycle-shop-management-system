...
export function Sidebar() {
  ...
  return (
    <aside
      ...
      className="hidden md:flex fixed top-16 left-0 bottom-0 w-64 bg-zinc-950 border-r border-zinc-800 flex-col z-30 select-none"
    >
      {/* Dynamic Nav Items */}
      <nav
        ref={navRef}
          ...
          return (
            <div key={item.href} className="space-y-1">
              ...
              <Link
                ...
                className={`flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-sm font-medium transition-colors relative group ${
                  isActive
                    ? "nav-active-item bg-zinc-900 text-white border border-zinc-700 font-bold"
                    : "text-zinc-400 hover:text-white hover:bg-zinc-900"
                }`}
              >
                {isActive && (
                  <span className="nav-active-bar absolute left-1.5 top-2 bottom-2 w-1 rounded-full bg-emerald-500" />
                )}
                <Icon className={`w-4 h-4 shrink-0 transition-colors ${isActive ? "text-emerald-400" : "text-zinc-400 group-hover:text-zinc-200"}`} />
      ...
      <div className="shrink-0 p-3.5 border-t border-zinc-800 bg-zinc-900 space-y-2">
        ...
        <div className="flex items-center justify-center gap-3 pt-1 text-[10px] text-zinc-400 font-medium">
