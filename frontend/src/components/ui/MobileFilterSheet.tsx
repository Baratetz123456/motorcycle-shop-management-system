...
export function MobileFilterSheet({
  ...
  const content = (
    <div className="fixed inset-0 z-50 md:hidden flex flex-col justify-end">
      ...
      <div 
        className="relative z-10 w-full max-h-[88vh] bg-white border-t border-slate-200 rounded-t-3xl shadow-2xl flex flex-col overflow-hidden animate-in slide-in-from-bottom duration-200"
      >
        {/* Top Grab Handle */}
          ...
          <div className="flex items-center gap-2">
            {onReset && (
              <button
                type="button"
                onClick={onReset}
                className="px-2.5 py-1 text-xs text-slate-500 hover:text-slate-900 transition-colors flex items-center gap-1 font-semibold rounded-lg hover:bg-slate-100"
            ...
            <button
              type="button"
              onClick={onClose}
              className="p-1.5 rounded-xl bg-slate-100 text-slate-500 hover:text-slate-900 border border-slate-200 transition-colors"
              title="Close filters"
            >
              <X className="w-4 h-4" />
            </button>
        ...
        <div className="flex-1 min-h-0 overflow-y-auto p-5 space-y-5 touch-pan-y overscroll-contain text-slate-700">
          {children}
        </div>
        ...
        <div 
          ...
          className="p-4 border-t border-slate-200 bg-white shrink-0 sticky bottom-0 z-20 shadow-lg"
        >
          <button
            type="button"
            onClick={onClose}
