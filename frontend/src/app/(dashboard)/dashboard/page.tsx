...
export default function ExecutiveDashboardPage() {
  ...
  return (
    <div className="min-h-full bg-zinc-950 text-zinc-100 font-sans p-4 sm:p-6 lg:p-8 overflow-y-auto w-full">
      ...
      <div className="pb-6 border-b border-zinc-800 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-3">
            ...
            <span className="px-2.5 py-0.5 rounded-full text-[10px] font-bold tracking-wider uppercase bg-zinc-800 text-zinc-300 border border-zinc-700 flex items-center gap-1.5">
        ...
        <div className="flex items-center gap-2.5 flex-wrap">
          <button
            ...
            className="px-4 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 active:bg-emerald-700 text-white font-bold border border-emerald-500 text-xs transition-all flex items-center gap-2 active:scale-[0.98]"
          >
          ...
          <button
            ...
            className="px-4 py-2.5 rounded-xl bg-zinc-900 hover:bg-zinc-800 text-zinc-300 hover:text-white border border-zinc-800 text-xs font-bold transition-all flex items-center gap-2 active:scale-[0.98]"
          >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 py-6 border-b border-zinc-800 mb-8 divide-y lg:divide-y-0 lg:divide-x divide-zinc-800">
        ...
        >
          ...
          <div className="text-2xl sm:text-3xl font-black text-zinc-100 font-mono">
            {activeWorkshopJobs.length} <span className="text-sm font-sans font-bold text-zinc-400">Bikes</span>
          </div>
          <div className="flex items-center gap-1 text-[11px] text-zinc-400 font-medium group-hover:text-zinc-200 mt-1">
        ...
        >
          ...
          <div className="text-2xl sm:text-3xl font-black text-zinc-100 font-mono">
            {lowStockItems.length} <span className="text-sm font-sans font-bold text-zinc-400">Parts</span>
          </div>
          <div className="flex items-center gap-1 text-[11px] text-zinc-400 font-medium group-hover:text-zinc-200 mt-1">
      ...
      <div className="flex md:hidden items-center gap-1.5 overflow-x-auto no-scrollbar pb-3 mb-6 border-b border-zinc-800">
        {[
        ...
        <div className={clsx("lg:col-span-2 space-y-8", mobileTab === "top" && "hidden md:block")}>
          ...
          <div className={clsx("space-y-4", mobileTab === "sales" && "hidden md:block")}>
            ...
            {activeWorkshopJobs.length === 0 ? (
              <div className="p-8 text-center border border-dashed border-zinc-800 rounded-2xl bg-zinc-900">
                <Bike className="w-8 h-8 text-zinc-600 mx-auto mb-2" />
                <p className="text-xs text-zinc-400 font-semibold">All workshop bays are currently clear.</p>
                <p className="text-[11px] text-zinc-500 mt-0.5">New intake jobs from the counter will appear here immediately.</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                {activeWorkshopJobs.slice(0, 4).map((job) => (
                  <Link
                    ...
                    className="p-4 rounded-2xl bg-zinc-900 border border-zinc-800 hover:border-zinc-700 transition-all group flex flex-col justify-between"
                  >
                    <div>
                      <div className="flex items-center justify-between gap-2 mb-2">
                        <span className="font-mono text-xs font-bold text-zinc-200 bg-zinc-800 px-2 py-0.5 rounded-lg border border-zinc-700">
                          {job.jo_number}
                        </span>
                        <span className="text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full border bg-zinc-800 text-zinc-300 border-zinc-700">
                          {job.status}
                        </span>
                      </div>
                    ...
                    <div className="pt-3 mt-3 border-t border-zinc-800 flex items-center justify-between text-xs">
                      ...
                      <ArrowUpRight className="w-4 h-4 text-zinc-400 group-hover:text-emerald-400 group-hover:translate-x-0.5 transition-all" />
                    </div>
                  </Link>
                ))}
              </div>
          ...
          <div className={clsx("space-y-4", mobileTab === "bays" && "hidden md:block")}>
            ...
            {completedSales.length === 0 ? (
              <div className="p-8 text-center border border-dashed border-zinc-800 rounded-2xl bg-zinc-900">
                ...
                <p className="text-xs text-zinc-400 font-semibold">No sales transactions recorded yet.</p>
              </div>
            ) : (
                ...
                {completedSales.slice(0, 5).map((sale) => (
                  ...
                  >
                    <div className="flex items-center gap-3.5">
                      <div className="w-10 h-10 rounded-xl bg-zinc-800 border border-zinc-700 flex items-center justify-center text-zinc-300 font-mono text-xs font-bold shrink-0">
                    ...
                    <div className="text-right">
                      ...
                      <span className="text-[11px] text-zinc-400 font-semibold group-hover:text-emerald-400">
                        Inspect Receipt
                      </span>
                    </div>
                  </Link>
        ...
        <div className={clsx("space-y-8", (mobileTab === "bays" || mobileTab === "sales") && "hidden md:block")}>
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-5 space-y-4">
            ...
            {lowStockItems.length === 0 ? (
              <div className="py-6 text-center text-xs text-zinc-400 font-semibold flex items-center justify-center gap-1.5">
              ...
              <div className="space-y-3">
                {lowStockItems.slice(0, 4).map((item) => (
                  <div
                    key={item.id}
                    className="p-3 rounded-xl bg-zinc-800/60 border border-zinc-700/80 flex items-center justify-between text-xs"
                  >
                    ...
                    <div className="text-right shrink-0">
                      <span className="px-2 py-0.5 rounded-md font-mono font-bold text-xs bg-zinc-800 text-zinc-300 border border-zinc-700">
