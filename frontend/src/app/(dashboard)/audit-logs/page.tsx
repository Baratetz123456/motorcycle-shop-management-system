...
export default function AuditLogsPage() {
  ...
  return (
    <div className="h-screen bg-zinc-950 p-8 flex flex-col overflow-hidden font-sans text-zinc-100">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6 flex-shrink-0">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-cyan-600 via-blue-600 to-indigo-600 hover:from-cyan-500 hover:to-blue-500 text-white rounded-xl font-bold shadow-lg shadow-cyan-500/20 transition-all text-sm disabled:opacity-50"
          >
      ...
      {/* Filter & Search Bar */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 p-4 bg-zinc-900/60 border border-white/10 rounded-2xl backdrop-blur-xl mb-6 flex-shrink-0">
        {/* Search */}
        ...
        {/* Counter */}
        <div className="flex items-center justify-end px-3 text-xs text-zinc-400 font-mono">
          Total Logs: <span className="font-semibold text-cyan-400 ml-1.5">{total}</span>
        </div>
      </div>

      {/* Audit Log Table (Expanded to full container space) */}
              ...
              {isLoading ? (
                ...
                logs.map((log) => (
                  <tr key={log.id} className="hover:bg-white/[0.04] transition-colors">
                    <td className="py-4 px-6 text-xs text-zinc-400 font-mono whitespace-nowrap">
                      {log.timestamp ? new Date(log.timestamp).toLocaleString() : "-"}
                    </td>
                    <td className="py-4 px-6 whitespace-nowrap">
                      <div className="flex items-center gap-2">
                        <span className="font-semibold text-zinc-200">{log.user_id ? log.user_id.slice(0, 8) + "..." : "Anonymous"}</span>
                        {log.user_role && (
                          <span className="px-2 py-0.5 text-[10px] font-bold rounded border bg-zinc-950 border-white/10 uppercase text-zinc-300">
                            {log.user_role}
                          </span>
                        )}
                      </div>
                    ...
                    <td className="py-4 px-6 text-zinc-300 max-w-xs truncate text-xs font-mono">{log.resource}</td>
                    <td className="py-4 px-6 text-zinc-400 text-xs font-mono">{log.ip_address || "127.0.0.1"}</td>
                    <td className="py-4 px-6 text-right whitespace-nowrap">
                      <button
                        onClick={() => setExpandedId(expandedId === log.id ? null : log.id)}
                        className="p-2 rounded-lg hover:bg-zinc-800 text-cyan-400 transition-colors"
